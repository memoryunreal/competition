...
class ExperimentUAV(object):
    r"""Experiment pipeline and evaluation toolkit for UAV dataset.
    
    Args:
        root_dir (string): 
            Root directory of UAV dataset where ``train``, ``val`` and ``test`` folders exist.
        save_dir (string): 
            Save directory of UAV dataset to save the experiment results.
        subset (string): 
            Specify ``train``, ``val`` or ``test`` subset of UAV.
        repetition (int): 
    ...
    def __init__(self, root_dir, save_dir, subset, repetition):
        super(ExperimentUAV, self).__init__()
        self.root_dir = root_dir
        self.subset = subset
        self.dataset = UAV(root_dir, subset)
    ...
    def run(self, tracker, visualize, save_img):
        """
        Run the tracker on UAV subset.
        """
        print('Running tracker %s on UAV...' % tracker.name)
        ...
        for s, (img_files, anno, restart_flag) in enumerate(self.dataset):
            ...
            # tracking loop
            boxes, times = tracker.track(seq_name, img_files, anno, restart_flag, visualize, seq_result_dir, save_img, method=None)
    ...
    def report(self, tracker_names):
        """
        Evaluate the tracker on UAV subset.
        """
        ...
        for name in tracker_names:
            ...
            for s in range(len(self.dataset.seq_names)):
                ...
                # read absent info
                absent_path = os.path.join(self.root_dir, 'attribute', 'absent','absent_{}.txt'.format(num))
    ...
    def plot_curves_(self, report_files, tracker_names, rep):
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1),
               title='Success plots on UAV (based on IoU)')
        ax.grid(True)
        fig.tight_layout()
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1),
               title='Success plots on UAV (based on DIoU)')
        ax.grid(True)
        fig.tight_layout()
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1),
               title='Success plots on UAV (based on GIoU)')
        ax.grid(True)
        fig.tight_layout()
        ...
        ax.set(xlabel='Normalized location error threshold',
               ...
               title='Normalized precision plots on UAV')
        ax.grid(True)
        fig.tight_layout()
